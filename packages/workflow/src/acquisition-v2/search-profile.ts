...
const UNIVERSAL_LAWS = [
  ...
  "② 画质不进搜索词:把 4K/1080P/蓝光 拼进关键词会过滤掉标题匹配、跑偏到同画质的错作品。画质只在召回后读标题判。",
  "③ count ≠ 相关性:读 top 标题确认是目标本体+全集,别只看数量。+年份是真人电影/真人剧的安全降噪/破重名键;但动漫(日漫/国漫)忌 +年份(归零或拉同名真人版)。",
  "④ 子类型词永不进搜索词:+美剧/+韩剧/+日剧/+国产剧/+番剧/+动画 几乎从不帮忙,常把召回打到 0 或顶噪声。唯二例外:国漫的 +国漫(发布标题真带此 tag,用于和真人版同名消歧)、切尔诺贝利 +美剧(裸名全 0 的孤例兜底)。",
  "⑤ 搜索关键词语言跟字幕偏好:偏好中文字幕(默认)→ 外国片(美/日/韩剧、动漫)一律用【中文译名】首搜——中文名资源带中字、且在本站召回更好;偏好原文 → 才用原名/英文名首搜(召回大但多无中字)。原名只作中文名复搜仍 0 的兜底,且兜底要在结果里挑带中字的,没有则按字幕偏好判弱覆盖。",
].join("\n");
